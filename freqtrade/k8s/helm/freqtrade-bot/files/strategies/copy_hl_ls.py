...
ADDRESS_TO_TRACK_TOP = "{{ .Values.exchange.walletToCopy }}"
...
class PositionTracker:
                    ...
                    for pos in positions:
                        writer.writerow(
                            [
                                ...
                                pos.margin_used,
                                pos.timestamp,
                                self._timestamp_to_human(pos.timestamp),
                            ]
                        )
                ...
                for change in self.changes_log:
                    writer.writerow(
                        [
                            ...
                            change.old_size or "",
                            change.new_size,
                            change.old_position_value or "",
                            change.new_position_value,
                            change.timestamp,
                            change.human_time,
                            ]
                    )
        ...
        try:
            with open(self.changes_file, "r", newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    old_pos_value = (
                        float(row["old_position_value"])
                        if row["old_position_value"]
                        else None
                    )
    ...
    def _load_data(self) -> None:
        ...
        if self.last_positions or self.changes_log:
            logger.info(
                f"Loaded tracking data: {len(self.last_positions)} current positions, "
                f"{len(self.changes_log)} historical changes from {self.data_dir}/"
            )
        else:
            logger.info(
                f"No existing data found. Starting with fresh tracking data in {self.data_dir}/"
            )
    ...
    def export_to_json(self, filename: str = None) -> str:
        ...
        if filename is None:
            filename = os.path.join(
                self.data_dir, f"export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def _detect_changes(
        ...
        changes = []
        timestamp = (
            list(current_positions.values())[0].timestamp
        ...
        for coin, current_pos in current_positions.items():
            if coin not in self.last_positions:
                ...
                # Check for significant modifications (leverage, entry price changes)
                        ...
                        abs(current_pos.leverage - old_pos.leverage) > 1e-8
                        or abs(current_pos.entry_price - old_pos.entry_price) > 1e-6
                ):  # Higher threshold for entry price
                    change = PositionChange(
                        coin=coin,
    ...
    def print_changes(self, changes: List[PositionChange]) -> None:
        ...
        for change in changes:
            position_info = self._get_position_info(
                change.new_size if change.new_size != 0 else change.old_size
            )
            ...
            if change.change_type.startswith("opened"):
                ...
                logger.info(
                    f"  New {direction} position: {abs(change.new_size):,.4f} (${change.new_position_value:,.2f})"
                )
            elif change.change_type == "closed":
                old_direction = "LONG" if change.old_size > 0 else "SHORT"
                logger.info(
                    f"  Closed {old_direction} position: {abs(change.old_size):,.4f} (was ${change.old_position_value:,.2f})"
                )
            elif change.change_type == "flipped":
                old_direction = "LONG" if change.old_size > 0 else "SHORT"
                new_direction = "LONG" if change.new_size > 0 else "SHORT"
                logger.info(
                    f"  Position flipped from {old_direction} to {new_direction}"
                )
                logger.info(f"  Size: {change.old_size:,.4f} → {change.new_size:,.4f}")
                logger.info(
                    f"  Value: ${change.old_position_value:,.2f} → ${change.new_position_value:,.2f}"
                )
                ...
                logger.info(
                    f"  Size: {change.old_size:,.4f} → {change.new_size:,.4f} ({size_diff:+,.4f})"
                )
                logger.info(
                    f"  Value: ${change.old_position_value:,.2f} → ${change.new_position_value:,.2f} ({value_diff:+,.2f})"
                )
            elif change.change_type == "modified":
                direction = "LONG" if change.new_size > 0 else "SHORT"
                logger.info(
                    f"  {direction} position modified (same size: {change.new_size:,.4f})"
                )
                logger.info(
                    f"  Value: ${change.old_position_value:,.2f} → ${change.new_position_value:,.2f}"
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.changes_log:
            stats["first_change"] = self._timestamp_to_human(
                self.changes_log[0].timestamp
            )
            stats["last_change"] = self._timestamp_to_human(
                self.changes_log[-1].timestamp
            )
...
class COPY_HL_LS(IStrategy):
    global ADDRESS_TO_TRACK_TOP
    minimal_roi = {"0": 5000.0}  # Effectively disables ROI
    stoploss = -0.95
    timeframe = "5m"
    ...
    def GET_PERP_ACCOUNT_STATUS(self, address):
        ...
        try:
            ...
            if (
                    self._cached_perp_data is not None
    ...
    def check_print_positions_summary(self):
        ...
        try:
            ...
            if perp_data:
                ...
                logger.info(
                    f"Scale Factor:        {scale_factor:.6f}x (inverted {1.0/scale_factor:.1f}x )"
                )
                logger.info("-" * 50)
            else:
            ...
            if self.current_positions_to_copy:
                for coin, position in self.current_positions_to_copy.items():
                    ...
                    logger.info(
                        f"  {coin:>8} | {position_type:>5} | Size: {size:>12.4f} | "
            ...
            if self.my_open_positions:
                for trade in self.my_open_positions:
                    ...
                    position_type = "LONG" if trade.is_short == False else "SHORT"
                    logger.info(
                        f"  {coin:>8} | {position_type:>5} | Stake: ${stake_amount:>10.2f} | "
                        f"Value: ${position_value:>10.2f} ({ratio_pc:>5.2f}%) | "
            ...
            if self.current_positions_to_copy and self.my_open_positions:
                ...
                if matching:
                    ...
                    for coin in matching:
                        ...
                        my_trade = next(
                            t
                            for t in self.my_open_positions
                            if t.pair.replace("/USDC:USDC", "") == coin
                        )
                        ...
                        copied_margin = float(copied_pos.margin_used)
                        expected_value = copied_margin * scale_factor * float(my_trade.leverage)
                        diff_pc = (
                            ((my_value - expected_value) / expected_value * 100)
                            if expected_value > 0
                            else 0.0
                        )
                ...
                if should_have:
                    logger.info(
                        "  Missing positions (should open if in whitelist and significant size):"
                    )
                    for coin in should_have:
                        ...
                        expected_ratio_pc_my = (
                            (expected_value / my_account_value * 100.0)
                        ...
                        ratio_pc_copied = (
                                pos.position_value / copied_account_value * 100.0
                        )
                        significant = (
                            "✓" if ratio_pc_copied >= self.change_threshold else "✗"
                        )
                ...
                if shouldnt_have:
                    ...
                    for coin in shouldnt_have:
                        ...
                        position_type = (
                            "LONG" if my_trade.is_short == False else "SHORT"
                        )
                        logger.info(
                            f"    {coin:>8} | {position_type:>5} | ${my_value:>8.2f}"
                        )
    ...
    def bot_start(self, **kwargs) -> None:
        ...
        # Snapshot positions that already exist on the copied account at startup.
        # These will be ignored — only positions opened AFTER the bot starts will be copied.
        self._initial_positions = set()
        try:
            info = Info(constants.MAINNET_API_URL, skip_ws=True)
            perp_data = info.user_state(self.ADDRESS_TO_TRACK)
            if perp_data:
                here = Path(__file__).resolve().parent / "position_data"
                tmp_tracker = PositionTracker(data_dir=here)
                positions = tmp_tracker._extract_positions(perp_data)
                self._initial_positions = set(positions.keys())
                logger.info(f"Initial positions snapshot (will NOT be copied): {self._initial_positions}")
        except Exception as e:
            logger.error(f"Failed to snapshot initial positions: {e}")
    ...
    def bot_loop_start(self, current_time: datetime, **kwargs) -> None:
        ...
        try:
            ...
            if perp_data is None:
                ...
                self.copied_account_position_changes = tracker.track_positions(
                    perp_data
                )
            ...
            # When a pre-existing position gets closed, remove it from the initial snapshot
            # so that if the trader re-opens it later, we will copy it.
            if self.copied_account_position_changes:
                for chg in self.copied_account_position_changes:
                    if chg.change_type == "closed" and chg.coin in self._initial_positions:
                        self._initial_positions.discard(chg.coin)
                        logger.info(
                            f"Removed {chg.coin} from initial positions snapshot (closed by copied account). "
                            f"Will copy if re-opened."
                        )
        ...
        self._update_whitelist_from_copied_positions()
        self.matching_positions_check_output = self.check_print_positions_summary()

    def _update_whitelist_from_copied_positions(self):
        """Dynamically set the pair whitelist based on new copied positions
        (ignoring pre-existing ones) plus our own open trades."""
        try:
            desired_pairs = set()

            # Add coins the copied wallet holds that were opened AFTER bot start
            if self.current_positions_to_copy:
                for coin in self.current_positions_to_copy:
                    if coin not in self._initial_positions:
                        desired_pairs.add(f"{coin}/USDC:USDC")

            # Keep our own open trades so exit signals can still fire
            if self.my_open_positions:
                for trade in self.my_open_positions:
                    desired_pairs.add(trade.pair)

            if not desired_pairs:
                return  # keep current whitelist as-is

            current_wl = set(self.dp.current_whitelist())
            if desired_pairs == current_wl:
                return

            pair_list = sorted(desired_pairs)
            self.config['exchange']['pair_whitelist'] = pair_list
            self.dp._pairlists._whitelist = pair_list
            logger.info(f"Whitelist updated ({len(pair_list)} pairs): {pair_list}")
    ...
    def populate_indicators(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if (
                not self._got_perp_data_account_state_successfully
        ):  # skip (do nothing) if API call to get perp data copied account state failed
            return df
            ...
            for chg in self.copied_account_position_changes:
                if coin_ticker in chg.coin:
                    copied_account_value = float(
                        perp_data["marginSummary"]["accountValue"]
                    )
                    position_value_in_copied_account = float(
                        chg.new_position_value
                    )  # in USDC
                    ratio_pc = (
                            position_value_in_copied_account / copied_account_value * 100.0
                    )

                    # Handle both long and short opens
                    if "opened_long" == chg.change_type:
                        # Skip positions that existed before the bot started
                        if coin_ticker in self._initial_positions:
                            logger.info(
                                f"Skipping {coin_ticker} LONG — position existed before bot start"
                            )
                            continue
                        if ratio_pc > self.change_threshold:
                            df["signal"] = 1  # Long entry signal
                            return df
                        else:
                            logger.info(
                                f"Not opening long position on {coin_ticker} because position size in copied account is too small compared to the copied account equity ({ratio_pc:.2f} , less than {self.change_threshold}%)"
                            )
                    elif "opened_short" == chg.change_type:
                        # Skip positions that existed before the bot started
                        if coin_ticker in self._initial_positions:
                            logger.info(
                                f"Skipping {coin_ticker} SHORT — position existed before bot start"
                            )
                            continue
                        if ratio_pc > self.change_threshold:
                            df["signal"] = -1  # Short entry signal
                            return df
                        else:
                            logger.info(
                                f"Not opening short position on {coin_ticker} because position size in copied account is too small compared to the copied account equity ({ratio_pc:.2f} , less than {self.change_threshold}%)"
                            )
                    elif "closed" in chg.change_type:
                        df["signal"] = 0  # Exit signal for both long and short
                        return df
        else:  # Handle missed entries/exits when no changes detected
            df = self._check_missed_entry_or_exit(coin_ticker, df)

        # check if position direction mismatch -> send exit signal
        df = self.check_position_direction_mismatch(df, coin_ticker)
    ...
    def _check_missed_entry_or_exit(self, coin_ticker, df):
        ...
        try:
            ...
            my_current_opened_tickers = [
                tr.pair.replace("/USDC:USDC", "") for tr in my_trades
            ]

            # Check for missed entries (both long and short)
            # Skip positions that existed before the bot started
            if coin_ticker in self.current_positions_to_copy:
                if coin_ticker in self._initial_positions:
                    logger.info(
                        f"Skipping missed entry for {coin_ticker} — position existed before bot start"
                    )
                elif coin_ticker not in my_current_opened_tickers:
                    size = float(self.current_positions_to_copy[coin_ticker].size)
                    is_short = size < 0.0
                    if self._is_position_significant(coin_ticker):
                        if is_short:
                            df["signal"] = -1  # Short entry signal
                            logger.info(
                                f"Missed short entry detected for {coin_ticker}. Sending short entry signal."
                            )
                        else:
                            df["signal"] = 1  # Long entry signal
                            logger.info(
                                f"Missed long entry detected for {coin_ticker}. Sending long entry signal."
                            )

            # Check for missed exits
            #   not in current positions to copy, but somehow in my current position
            if (
                    coin_ticker not in self.current_positions_to_copy
                    and coin_ticker in my_current_opened_tickers
            ):
                df["signal"] = 0
                logger.info(
            ...
            if (
                    coin_ticker in self.current_positions_to_copy
                    and coin_ticker in my_current_opened_tickers
            ):
    ...
    def _is_position_significant(self, coin_ticker):
        ...
        try:
            ...
            min_threshold = copied_account_value / (
                    100.0 / self.change_threshold
            )  # 1% threshold
    ...
    def check_position_direction_mismatch(self, df, coin_ticker):
        """
        Check if my position direction doesn't match the copied position direction
        """
        if coin_ticker in self.current_positions_to_copy:
            my_trades = Trade.get_trades_proxy(is_open=True)
            for trade in my_trades:
                if trade.pair.replace("/USDC:USDC", "") == coin_ticker:
                    copied_size = float(
                        self.current_positions_to_copy[coin_ticker].size
                    )
                    copied_is_short = copied_size < 0.0
                    my_is_short = trade.is_short

                    # If directions don't match, exit the position
                    if copied_is_short != my_is_short:
                        df["signal"] = 0
                        logger.info(
                            f"Position direction mismatch on {coin_ticker}. Copied: {'SHORT' if copied_is_short else 'LONG'}, Mine: {'SHORT' if my_is_short else 'LONG'}. Exiting to realign."
                        )
                        return df
        return df
    ...
    def custom_stake_amount(
        ...
        try:
            ...
            if position_value_in_copied_account is None:
                if coin_ticker in self.current_positions_to_copy:
                    position_value_in_copied_account = self.current_positions_to_copy[
                        coin_ticker
                    ].position_value
                    copied_position_size = float(
                        self.current_positions_to_copy[coin_ticker].size
                    )

            if position_value_in_copied_account is None or copied_position_size is None:
                logger.warning(f"No position data found for {coin_ticker}")
                return None

            # Determine position direction
            is_short_position = copied_position_size < 0.0
            position_type = "SHORT" if is_short_position else "LONG"

            # Validate that the side matches the position direction
            if (side == "short" and not is_short_position) or (
                    side == "long" and is_short_position
            ):
                logger.warning(
                    f"Position direction mismatch for {coin_ticker}: side={side}, copied_position={'SHORT' if is_short_position else 'LONG'}"
                )
                return None
            ...
            if ratio_pc < self.change_threshold:
                logger.info(
                    f"Not opening {position_type} position on {pair} because position size in copied account is too small ({ratio_pc:.1f}% < {self.change_threshold}%)"
                )
                return None
            ...
            logger.info(
                f"Calculated stake for {position_type} {pair}: {returned_val} (copied value: ${position_value_in_copied_account:.2f}, scale: {scale_factor:.4f})"
            )
            return returned_val
    ...
    def adjust_trade_position(
        ...
        if self._time_of_change is not None:
            if datetime.now() > self._time_of_change + timedelta(
                    seconds=self._cooldown_seconds_after_position_change
            ):
                self._is_cooldown_after_position_change = False

        if self._is_cooldown_after_position_change:
            logger.info(
                f"Not doing position size change because of the cooldown of {self._cooldown_seconds_after_position_change} seconds."
            )
            return None
        ...
        try:
                ...
                for chg in self.copied_account_position_changes:
                    if coin_ticker == chg.coin:
                        ...
                        change_ratio_pc = (
                                ...
                                / copied_account_value
                                * 100.0
                        )
                        if change_ratio_pc < self.change_threshold:
                            logger.info(
                                f"Not adjusting position on {trade.pair} because position change in copied account is too small ({change_ratio_pc:.1f}% < {self.change_threshold}%)"
                            )
                            return None

                        # Determine position direction for logging
                        position_type = "SHORT" if trade.is_short else "LONG"
                        old_size = (
                            float(chg.old_size) if chg.old_size is not None else 0.0
                        )
                        ...
                        if "increased" in chg.change_type:
                            logger.info(
                                f"Increasing {position_type} position on {trade.pair}: +${adjustment_amount:.2f} (copied size: {old_size:.4f} -> {new_size:.4f})"
                            )
                            return adjustment_amount
                        elif "decreased" in chg.change_type:
                            logger.info(
                                f"Decreasing {position_type} position on {trade.pair}: -${adjustment_amount:.2f} (copied size: {old_size:.4f} -> {new_size:.4f})"
                            )
                            return -1.0 * adjustment_amount
            ...
            if self.matching_positions_check_output:
                for pos in self.matching_positions_check_output:
                    logger.info(
                        f"{pos['coin']} → Difference: {pos['diff_pc']:.2f}%   (my total value: {pos['my_value']:.1f}) ; ||>{self.adjustement_threshold:.0f}% will trigger a size correction."
                    )
                    if pos["coin"] == coin_ticker:
                        if abs(pos["diff_pc"]) > self.adjustement_threshold:
                            position_type = "SHORT" if trade.is_short else "LONG"
                            delta_stake = (
                                    pos["my_value"] / (1.0 + pos["diff_pc"] / 100.0)
                                    - pos["my_value"]
                            )
                            adjustment_amount = delta_stake / trade.leverage - dust_USDC

                            action = "Increasing" if delta_stake > 0 else "Decreasing"
                            logger.info(
                                f"{action} {position_type} position on {trade.pair} to correct size mismatch: ${adjustment_amount:.2f} (diff: {pos['diff_pc']:.1f}%)"
                            )
