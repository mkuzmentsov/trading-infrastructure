...
2026-09-13 ~23:40K | ⚠️ BUG in the new archive cycle, caught by the user asking "is the job working now?" | conversion WAS running (0 unknown rows) but the PRUNE never fired: the verify condition was `written >= nread - unknown`, which ignores the deliberately-dropped depth-diff rows, so `ok` was False on every file containing them and raw was re-converted every cycle forever (symptom: raw hours 14-18 surviving repeated cycles). FIX: `ok = written + ndiff + unknown >= nread` — every raw row must be accounted for by one of the three dispositions. Verified: venue raw 43 files -> 0, pq-venue 112MB | also fixed the cycle summary, which reported total local raw (2.3G, mostly the PM mrec/mrecev tapes this cycle deliberately does not touch) as if it were leftover venue raw | LESSON: a verify-then-delete guard that is too strict fails SILENTLY as unbounded disk growth — assert the guard actually fires at least once when you ship it

## 2026-09-14 03:45K — Day close 2026-09-13 (UTC): −$39.70 (54-3)
5m fleet FINAL: bnb 4-0 +0.84 | btc 6-0 +2.18 | doge 31-0 +2.14 | eth 0-0 | hype 0-0 | sol 13-3 −44.86 | xrp 0-0.
Whole day = sol: two $24 bid-collapse traps (−$47.5) + one small partial; rest of fleet green. Mid-day: sol halt DISABLED per user (999, deploy rev 22) — the §57 cooldown would have benched it after loss #2. doge quietly 31-0 in data mode. eth/hype/xrp 4th silent day. Balance 03:44K $408.86. Era 15 days ≈ +$268.
2026-09-14 ~06:00K | ⭐⭐ OVERNIGHT MULTI-AGENT HUNT (user: role agents, AI models, find profitable strats, 5m only) — workflow, 37 agents, 4.16M tokens, 0 errors: 6 specialists → 24 candidates → top 10 × 3 adversarial validators → synthesis | RESULT: **0 deployable**, and the real output is a correction to our OWN baseline: the fleet earns **$8.01/day NET not ~$19** (the pod ledger never charges the taker fee, −$124.53/29d), day-clustered **t=1.20**, same-price null z=+1.57, **top-5 fills = 84% of net**, ex-top-5 **$1.28/day**, 0.17 dislocations/day ⇒ every candidate was competing against an unproven baseline; t=2 needs ~80 more days | SURVIVORS (both negatives, both valuable): (1) latency/nowcast/co-location CLOSED BY CEILING — perfect zero-lag oracle flips 1 gated bar in 16 days ≈ $0.67/day; ~99% of gated error is unobserved FUTURE ticks not relay lag; depth20 imbalance IS signal (p<1e-4) but 0.06 bps vs 4.9 bps median est = unbuyable; (2) Kelly/edge/price-proportional sizing REFUTED — advantage is the same 5 unrepeatable fills, tape-capped Kelly −$14..−$24 vs flat +$218 ⇒ keep $24 flat | near-miss KILLED: excising the 0.90-0.975 seen_ask band (n=965, −$156.51, t=−2.05) is an ERA ARTIFACT — pre-09-01 −$178, post-09-01 **+$22** | 8 more refutations logged in the doc (deferExec, relay-gap veto, Binance-lead residual, tl≤14 triple, k/n envelope gate, HYPE 3x liquidity, ...) | ⭐ NEW SCORING RULE: drop fills with req_px−avg_px≥0.005 before judging any candidate — removes $151 of $232; "tl≤14 lane"/"cheap-ask lane"/"sweeps" are the same dollar relabelled | ⭐ BUGS #43/#44 FIXED in panel.py: (a) look-ahead — the window clamp made 100% of tlk>=63 rows average a FUTURE tick (verified on all 1.65M rows), now NaN; (b) panel averaged only delivered ticks while the live bot FORWARD-FILLS (core/rtds.py window_mean) — now matched, 63.8% of rows changed, median |Δ| 0.044 bps; ⚠️ I could NOT reproduce the validator's claimed +0.91pp accuracy gain (my proxy gate moved the other way: 92.39→91.55 @tl12) — the fix is justified by FIDELITY not accuracy; lw3.py/lw4.py results predating today are UNLANDED | OPEN (needs user approval, touches a live bot): log the full favourite ask ladder + FAK fill breakdown at fire time in PF_TE_WHALE_ORDER — the only lever with a plausible multiple on $8/day, since 84% of money is dislocations whose available depth we never recorded
2026-09-14 ~20:2xK | user: "use mrec parquet and breakdown this market doge-updown-5m-1789403700" | single-bar autopsy, 16:35-16:40 UTC, pulled hour-16 raw (not yet drained) + scoped ev2pq/snap2pq/recon/panel | the bar was doge's **only live loss of the day: −$3.65** | margin **−0.571 bps** (DOWN), `PF_TE_VERIFY` strike/close err **0.0 bps**, Chainlink capture 58/59 + 59/59 — a near-tie we can label with certainty | taker-side aggregate for the whole market **−$47.73** (biggest loser NOT us: 36.67sh UP @0.95 at tl16 = −$34.96) | ⭐ MECHANISM: a **6 bps Chainlink spike that fully reverted inside the window** — running mean −2.65 (tl30) → −0.73 (tl14) → −0.57 final, never positive; but `_estimate(H1)` = `rtds.window_mean` **forward-fills the undelivered tail at the newest tick**, so 13 of 60 seconds were filled at the +6.22 bps PEAK ⇒ `est_bps +0.504` vs `eff_thresh` exactly **0.500** (tl=14 = THRESH_ANCHOR, slope term zero) — **fired by 0.004 bps**, blocked one second either side (+0.488/+0.069). Reproduced the logged value to the digit from `cl.parquet` (obs 47, tot 60) | the book flipped in **ONE 10Hz row**: tl 14.12 UP 0.92/0.93 → tl 14.00 UP 0.10/0.72; we bought the 0.72 | ⚠️⚠️ **BUG #45 — #44 was only HALF-fixed**: `window_mean` fills to `end−3` **unconditionally** (tot=60 always), `panel.py` truncates at the relay frontier (`b=min(end−3,cl_ts+1)`) ⇒ **different estimators**. This bar @tl14: **panel −0.901 vs live +0.504** (panel never sees the fire). Thin scope (doge, 21 bars, 378 rows tl3-20): median |Δ| **0.386 bps**, p90 1.21, side disagreement **2.4%** against a 0.5 bps gate. ⭐ panel is QUIETER and BETTER than the bot — it cannot make the spike-extrapolation error, so **every panel-scored gate has been flattered**; fix = fill to end−3 always | 🟡 freshness lever in a NEW form (estimator input, not a race): swapping the live Binance deviation (+2.66, already reverted) for the stale Chainlink tail (+6.22) gives **−0.198 ⇒ no fire**. ⚠️ n=1 selected because it lost; generic nowcasting is priced at $0.67/day by the perfect-oracle ceiling — the **tail-only** version is narrower and untested, NOT proposed | 🟡 the SHIP-verdict [[bid-drop-veto]] (still absent from the pods) **blocks** on the fire row (`dB6 −0.42`, seen_ask 0.72<0.98) and **allows** one 10Hz row earlier (`dB6 +0.40`) — the doc's "instrument gap" risk made concrete; argues for the `PF_TE_BIDDROP` log-only arm, not for shipping blocking | footnote: the §37 delay cost ~$3.74 here (tl29.9 DOWN@0.98 would have made +$0.09) — logged so nobody re-derives it; at ~30:1 payoff a single bar carries no information | new doc `docs/bar-autopsy-doge-20260914-1635.md`; 2 README rows; bug ledger #45
2026-09-14 ~20:5xK | ⚠️ CORRECTION to the entry above (found while building the dashboard, reading brec at FULL resolution instead of 1-second buckets) | the freshness counterfactual figures were wrong: Binance did NOT "give back two-thirds 2s early" — it held +6.68/+7.86 bps until **tl 16.76**, then **collapsed to +0.74 bps inside ~100ms** (tl 14.13→14.02), and the **last update at or before the fire (tl 14.016) reads +0.741 bps**. Filling the H1 tail with that gives **−0.610 bps (no fire)**, within 0.04 bps of the truth (−0.571) — not the −0.198 first published | ⭐ the PM book collapsed in the SAME 10Hz row, so the makers were tracking Binance and our bot was the only participant still pricing off the stale Chainlink peak | ⚠️⚠️ **the lever is knife-edge**: the same substitution made ONE SECOND EARLIER reads **+0.933** (fires harder, still loses). It is only worth anything at sub-second freshness ⇒ **a 1-second-bucketed backtest of this idea is not merely noisy, it inverts the sign** — my own first pass did exactly that. Bucketing a 100ms event at 1Hz averages across the crash | doc §2/§4a/§5c + README row corrected in place
2026-09-14 ~21:1xK | user: "save this as a skill or agent whatever is better / make colors distinguishable - currently it's a mess" | ⭐ NEW SKILL `.claude/skills/pm-bar-dash/` (SKILL.md + build_dash.py + assemble.py + template/) — one-bar autopsy end-to-end: resolve slug→ws, pull the mrec hour if the drain hasn't run, scoped parquet via the COMMITTED ev2pq/recon.py, pod event log for entries, raw Binance bookTicker, → dash.json → standalone 2-chart page. Auto-fills every narrative slot so a bare run renders complete; notes take HTML. Proven on a 2nd coin+bar (btc 1789401600, 874 prints, −12.5 bps) | ⭐ COLOUR FIX, and the root cause was ENCODING not palette: the chart drew BOTH tokens, but `ua ≡ 1−db` means that is the same information twice, mirrored — now plots ONE token (switchable), other-token trades mapped to 1−px. Two hues only (ask blue / bid orange, validated all-pairs both modes); ⚠️ status red/green as a LINE colour FAILS CVD against the bid orange (ΔE 4.1 deutan, 10.8 normal) → entries are ink rules with the status chip beside the *label*; prints are ink triangles (direction by SHAPE) | bugs found+fixed while building: global `svg{width:100%}` blew up inline legend icons; per-label clamp silently re-collapsed the de-collided end-label stack (clamp the STACK, not each label); fixed-6dp prices are nonsense off sub-dollar coins (adaptive sig-digits now) | ⭐ `PF_TE_VERIFY` now OUTRANKS our recon in the builder: recon averages only recorder-received ticks and read −0.602 vs the venue's −0.571 (~0.03-0.5 bps error; side always agreed) | 8 gotchas written into SKILL.md incl. #1 NEVER bucket Binance to 1s and #2 anchor on the FIRE row
2026-09-14 ~21:3xK | user: "investigate this one now btc-updown-5m-1789393200" | first run of the new `/pm-bar-dash` skill on a cold bar — worked end to end | ⭐⭐ **−$20.80 on ONE clip = btc's ENTIRE loss for the day** (8 fills, 7 wins, day −$18.96; ex-this-bar **+$1.84**) | margin **+0.0625 bps** (UP) — thinnest we have ever autopsied | ⚠️ **our recon got the SIDE WRONG** (−0.0836 DOWN vs venue +0.0625 UP): `fobs` 53/59, six Chainlink ticks missing from the final window — the [[binance-chainlink-divergence]] unlabelable slice, concrete; `PF_TE_VERIFY` precedence (skill gotcha 3) is what saved the page | ⭐ MECHANISM — and it is NOT yesterday's estimator artefact: H1 at the fire was **−1.947 off 35 REAL ticks, an honest read**, reproduced to the digit; BTC then rallied **+6.6 bps in the last 18s** (cl −1.87 → +1.68 in ONE second at tl 18) and dragged the 60s average across zero | ⭐⭐ **but Binance had the whole move AT THE FIRE INSTANT: +6.93 bps vs the Chainlink tick's −1.87 — 8.8 bps apart, and already there ~2s**. Filling H1's 18s unobserved tail with the live Binance dev gives **+0.704 vs eff_thresh 0.707 ⇒ NO FIRE** (and any fire is UP, the winner); truth +0.049 | ⭐ the sweep was a FALLING KNIFE not a windfall: book was **0.98/0.99 × 4,175 sh** one 10Hz row before the fire, **0.83/0.86 × 60** 110ms later, 0.11/0.14 by tl 15; the $20.80 dollar-FAK walked it and took **30.76 sh @ 0.676 vs a displayed 0.79** — the 14c "improvement" that tripped TOXIC_BRAKE WAS the collapse. Confirms [[btc-live-fill-ledger]]'s band split on a fresh case (≥0.98 sweeps +$37.46, ≤0.90 sweeps −$4.52; this one from 0.79) | 🟡 [[bid-drop-veto]] blocks this too (dB6 −0.14 / dB10 −0.08 / dB20 −0.12, ALL three, and NOT anchor-sensitive unlike the doge bar) ⇒ **2-for-2 on consecutive loss bars for a rule that has sat at SHIP for 4 days** | ⚠️⚠️ **BOTH bars were selected BECAUSE they lost** — this is mechanism, not evidence, and nothing is proposed for deploy | ⚠️ the tail-freshness lever is **NOT** what [[latency-ceiling-closed]] priced: that closed "existing ticks, zero relay lag" ($0.67/day) and explicitly found ~99% of gated error is *unobserved FUTURE ticks* — which is exactly this. Do not cite the ceiling to dismiss it, or these 2 bars to fund it | ⭐ THE MEASUREMENT: a veto is scorable on the pod ledger with **NO fill model** (it only ever removes a fire, whose real PnL is recorded). Binding constraint is TAPE — brec starts 09-13 17:40, so join `SNAP.spot`/`lead_bps` (10Hz, 7-day pod retention) to `PF_TE_WHALE_ORDER` | new doc bar-autopsy-btc-20260914-1340.md; 2 README rows; dashboard artifact 345f5625
2026-09-14 ~22:0xK | user: "bnb-updown-5m-1789384200" | ⛔⛔ **THE TAIL-FRESHNESS LEVER IS REFUTED — on the first bar I did not pick for its outcome, ~2h after I opened it** | this bar is a **+$9.06 WIN** (bnb's best fill of the day; bnb 16 fills, **0 losses**, +$23.13, this is 39% of it) | at the fire Binance read **+4.633 bps vs the Chainlink tick's −0.165** = 4.8 bps AGAINST our DOWN side — the SAME signature as the btc −$20.80 bar (8.8 against) — so the substitution reads **+0.582 vs a 0.5455 gate ⇒ it fires the OPPOSITE side and LOSES**; truth −0.2307 DOWN | ⭐ WHY IT FAILS: the substitution assumes **full, instant convergence of Chainlink to Binance, and Chainlink does not converge** — BNB reached only **+1.5 of Binance's +4.6** vs btc's +6.6 of +6.93. The convergence FRACTION is the entire model, it varies per bar and per coin, and fitting it on 3 bars is fitting noise. **Naive form DEAD, do not re-derive**; [[latency-ceiling-closed]] stands better than I allowed | ⭐ LESSON (the [[acknowledged-risk-is-the-deploy-plan]] shape again): I *named* the selection bias in both prior write-ups and opened the lever anyway. Naming a bias is not controlling for it — one control bar killed it. **Get the control bar BEFORE writing the lever up.** | ⭐⭐ what DID survive and got stronger: this bar is physically the SAME event as the btc loss — displayed ask solid, book collapses during the 574ms round trip, dollar-FAK sweeps the falling stack (**32.82 sh @ 0.7239 vs a displayed 0.99**, 27c improvement), TOXIC_BRAKE trips — but **+$9.06 instead of −$20.80**. The discriminator is the FAVOURITE'S OWN BID: btc 0.96→0.82 (dB6 **−0.14**, loss) / doge 0.52→0.10 (**−0.42**, loss) / bnb 0.80→0.93 (**+0.13**, WIN). Ask collapsing while the bid RISES = liquidity pull; falling with it = repricing. [[bid-drop-veto]] now **3-for-3 on direction**, and it leaves this winner alone TWICE over (out of scope by ask — `seen_ask 0.99` is exactly the ≥0.90 sweep-windfall class §9's ask-conditioning was added to protect — and dB6/10/20 all positive anyway) | ⚠️ still n=3, two selected-because-they-lost: the case is for the `PF_TE_BIDDROP` **log-only arm**, not for shipping blocking | misc: 3rd razor-thin gate crossing in 3 bars (est −0.550 vs eff_thresh **0.5455**, cleared by 0.0045 bps; doge cleared by 0.004) — worth a real look, NOT from 3 bars | same-bar cross-coin: at bar 1789393200 bnb won DOWN +$8.31 while btc LOST DOWN −$20.80 on the same clock | new doc bar-autopsy-bnb-20260914-1110.md; README row flipped to REFUTED; btc+doge docs stamped superseded; artifact 7bcd5e77
2026-09-14 ~22:4xK | user: "btc-updown-5m-1789241100" (2026-09-12 19:25 UTC — a bar from the PRE-brec era) | ⭐⭐ **+$30.10 on ONE clip = 83% of btc's whole 09-12 day** (9 fills, 0 losses, +$36.20); largest fill in any autopsy | margin +0.2647 bps UP; est at fire +0.919 vs gate 0.700 — the most comfortable of the four bars | ⭐⭐ **THE CANONICAL STALE-ASK WINDFALL**: UP was **0.91/0.92 × 1,920 sh** at T−20.12, **0.47/0.91** 110ms later; the $17.68 dollar-FAK (req 26 sh @ limit 0.68) swept the vacuum for **47.78 sh @ 0.3700 vs a displayed 0.67 = 44.8c improvement**, TOXIC_BRAKE tripped. Same 08-25 shape memory already records | ⚠️⚠️ **THIS BREAKS THE BID-DROP VETO'S CLEAN RUN**: ask 0.67 is IN scope (<0.98) and the favourite's bid was in free fall (dB6 **−0.350**, dB10 −0.400, dB20 −0.410) ⇒ **the veto DELETES a +$30.10 winner**. Scoreboard now **3-for-4, net −$5.65** (saves $3.65+$20.80, forfeits $30.10). NOT a refutation — it is precisely the fat-tail forfeit §9 pre-registered (*"34 winning fills worth +$219 go with it; the net is a difference of two fat tails; judge on loss-bars/day, NEVER on PnL"*) — but it must stop "2-for-2" being read as momentum toward shipping | ⭐ **and the BID DIRECTION fails as the sweep discriminator here**: it points DOWN on both the −$20.80 btc loss and this +$30.10 win. Nothing observable at the fire separates the two — same solid ask, same ~110ms collapse during the round trip, same FAK sweep, same brake. There the collapse was information (Binance showed the rally); here a liquidity vacuum | ⭐ **FIRST clearly pro-§37-delay bar**: the delay blocked a fire at T−29.9/ask 0.89/est +1.215; waiting for tl≤20 put the SAME $17.68 into a collapsed book — 47.78 sh instead of ~20, **+$30.10 instead of ~+$2** (≈+$28 earned). The gate has now been implicated 3 ways in 4 bars (−$3.74 doge, dropped btc into the collapse, +$28 here) = exactly the 30:1 noise §37 predicts; **none of these bars is evidence about the gate** | ⚠️ NO Binance series on this bar (brec starts 09-13 17:40) so the tail-freshness question cannot even be ASKED pre-09-13 — tape is now the binding constraint on every open question | ⚠️ **CORRECTION to my own framing**: I twice wrote that bars "were selected because they lost". Wrong about provenance — **all four bars were supplied by the USER**, I selected none. The real defect was narrower: the freshness lever was DERIVED on two bars that happened to be losses and TESTED on none | ⭐ SKILL FIX: `pod_entries` only grepped the live event log, so any bar older than today returned 0 entries — now reads the rotated `logs-training-events.jsonl.<date>.gz`; chart now labels OPEN/CLOSE inside each chip (value-only chips are ambiguous once they de-collide) | new doc bar-autopsy-btc-20260912-1925.md; README veto row replaced; artifact 86895aac
2026-09-14 ~23:1xK | user: "are these charts informative and how do we proceed?" | answered by running the question the 4 autopsies raised, on the LEDGER instead of on more bars | ⭐⭐ **THE LEDGER IS 10x BIGGER THAN WE HAVE BEEN TREATING IT**: `PF_TE_WHALE_ORDER`+`PF_TE_LIVE_SETTLE` across 7 pods = **8,487 fires / 4,384 joined fills / ~30 days (archives from 08-16) / +$241.29 NET fee-charged = $8/day**, independently reproducing [[fleet-baseline-corrected]] from a different join, with **no fill model and no tape** | ⚠️⚠️ **THE TAPE IS THE SCARCE RESOURCE, NOT THE LEDGER**: recorder pods carry **0 rolled hours** (archive_mrec drains every 30min) so the only mrec that exists is local **09-12→09-14, 3 days**; brec starts 09-13 17:40. ⇒ **ledger-only questions get 30 days, anything needing market-state-at-fire gets 3** — this reframes every open lead in the program and makes "log the ladder + fill breakdown IN THE BOT" the binding move | ⛔ **near-threshold fires are NOT worse** (3 of 4 autopsied bars cleared eff_thresh by <0.01 bps, which looked alarming): `|est|−gate` 0.00-0.05 is the **BEST** bucket per fill (+$0.102, n=670) vs +0.001 at 0.20-0.50 — **no conviction floor, closed** | ⭐ **SWEEP QUALITY = THE ASK YOU SWEPT FROM, not the depth**: deep sweeps (≥5c under display) from **≥0.98 → n=32, 84.4% win, +$103.08, +$3.22/fill, ex-best +$28.26, positive 16/20 days** (robust); 0.80-0.98 → −$76.22; <0.80 → +$13.79 @52%. Extends [[btc-live-fill-ledger]] (btc-only, 188 fills) to 7 coins/30 days, same sign | ⛔ **and I killed my own tempting negative in the same pass**: the 0.80-0.98 "trap band" is **ONE DAY** — 09-09 is −$70 of the −$76, ex-that-day ≈−$6/44 fills = noise (the §45-47 single-day fingerprint). Do NOT veto on it | ⭐ the 4 autopsied bars land exactly where the bands predict: bnb swept from 0.99 (+$9.06, windfall class); btc 13:40 from 0.79 and btc 09-12 from 0.67 — **both in the <0.80 band, one +$30.10 one −$20.80**, a 52%-win near-zero class, which is WHY nothing at the fire distinguished them | concentration unchanged: top-5 fills = **81%** of 30-day net, top-20 = 147% (the rest is net negative) | ⭐ VERDICT ON THE CHARTS: good MECHANISM + communication instrument (they made the ~110ms collapse visible at all, and forced the pipeline), but **every genuine finding this session came from arithmetic on the tape/ledger, not from looking at a chart** — marginal value of bar #5 is low | new doc strat-sweep-bands-20260914.md; README row
2026-09-15 ~00:3xK | user: "analyze all trades where we had binance available and see if we can create a new strategy (existing not touched), use parquets" | built a 30-hour panel (09-13 14:00→09-14 20:00 UTC, 6 coins, **2,006 settled bars, 54,391 decision-rows**) from pq-venue bookTicker + mrec; scripts committed at `tools/mrec/binance/{build,panel}.py` | ⛔⛔ **FIRST: MY 09-14 REFUTATION OF THIS LEVER IS VOID — it tested a MIS-SPECIFICATION.** I filled the TWAP tail with Binance's **absolute level**, which carries a **+4.71 bps systematic offset** vs the Chainlink aggregate ⇒ the substitution injects that bias and flips signs for free. The correct feature is `bin_move = bin(now) − bin(last chainlink tick)`, offset cancels by construction. On the bnb bar the "4.8 bps divergence" was **already there before the last tick** — the real move was **+0.555 bps** and H1' = −0.408 = **the winning side, kept**. Second self-correction on the same lever in two days; both from moving on n≈1 | ⭐⭐ **MECHANISM CONFIRMED AND ROBUST**: `corr(bin_move, tail_err) = +0.3996`, **OLS slope = 1.003** (intercept +0.003), R²=0.16, n=54,391. Bucket means sit on the diagonal; slope **0.94-1.16 across all 6 coins**, **0.96/1.01 across both days**. Chainlink IS a lagged aggregate of spot — the live estimator's forward-fill assumes the tail-error is 0 and it is instead ≈ bin_move | ⭐ estimator: mean |err| **0.3076 → 0.2862 bps (−7.0%)**; **where H1 and H1' disagree (96 rows / 64 bars) H1' is right 70.8% vs H1's 29.2%**, monotone to **8/8 at |H1'|>1 bps** | ⛔ **BUT NO TRADEABLE NEW LANE**: 2,194 decisive rows with a takeable ask net **−0.157 c/share**. ⭐ **the 0.98-0.99 band is 77% of the flow and priced to 0.05pp** (acc 98.76% vs ask 0.987) — an extremely efficient market and the whole reason this doesn't become a strategy. Binance IS worth **+0.456 c/sh** of estimator quality (H1-only −0.613 → −0.157), real and about half the gap, but doesn't cross zero | only positive band 0.90-0.95 (+1.76 c/sh, survives ex-best/ex-worst and both days) is **4 of 6 coins** at n=117 ⇒ not callable | ⛔ lane "bars the bot SKIPS that binance makes decisive" = 65 bars, 90.9% acc but avg ask 0.910 ⇒ **−0.46 c/sh** (control: H1's side on the same rows is 78.8%/−12.58, so the signal is genuinely Binance's) | ⛔ TWAP-damping over-reaction (book pricing a late spot move as an endpoint move when it's only worth ntail/60): **no coherent pattern**, signs flip across cells at n=21-54 | ⭐ VERDICT: no deployable new strategy from 30 hours; the value is ESTIMATOR quality on existing flow, which is out of scope by instruction. **The binding constraint is tape and it is SELF-SOLVING** — brec accumulates ~1.5GB/day, so re-run this exact script in a week at 5× the data rather than concluding from n=117 | new doc strat-binance-tail-20260914.md; README row replaced; bnb doc §1 stamped VOID
...
## 2026-09-17 03:45K — Day close 2026-09-16 (UTC): −$9.91 (67-3)
5m fleet FINAL: bnb 15-0 +4.32 | btc 5-0 +5.23 | doge 33-1 −2.32 (one $5 data-mode stake, capped clean) | eth 0-0 | hype 0-0 | sol 14-2 −17.14 | xrp 0-0.
First red day since 09-13 and mild: the whole deficit = sol's two morning cheap-fill traps (−$19.24 @0.65 brake-capped, −$4.71 tiny partial @0.78); the other 67 clips ground +$14 back. Zero halts/errors/abandons. eth/xrp 7th consecutive silent day, hype quiet again after its 09-15 windfall (user asked why — answered with evidence: pods fully alive, 30/24/32 DELAY near-misses, gate chain never aligned; healthy). Research: §73 shipped — bid-drop veto scored removal-only on live fills: vB −$121.5/14.6d (deletes 122 winners), vR −$21; CLOSED as a PnL fix, survives only as variance insurance; do-not-deploy. Balance 03:44K $452.36. Era 18 days ≈ +$324 booked (~$18/day booked).
