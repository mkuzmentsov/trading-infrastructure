> ✅ **Independently re-verified 2026-09-17**: `activeReferralDiscount` 0.04 master / 0.00 sub (the
> MCP trades the sub; the discount applies at BILLING, not in the quoted rate). And the load-bearing
> margin claim: `meta.collateralToken = 0` (USDC only), `clearinghouseState` and
> `spotClearinghouseState` are **separate endpoints over separate balance classes** ⇒ **spot does
> NOT collateralise perp. Both legs are funded in full.**
