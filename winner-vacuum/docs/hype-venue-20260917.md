> ✅ **Referral finding verified independently (2026-09-17):** `activeReferralDiscount` is **0.04 on
> the master** `0xD17E2499…` and **0.00 on the sub** `0x65ce540d…`, and **the MCP trades the sub**.
> ⚠️ Note `userCrossRate`/`userAddRate` read an identical 4.500/1.500 bps on BOTH — the discount is
> **applied at billing, not reflected in the quoted rate**, so comparing the rate fields alone
> would miss it. This agent's receipt evidence (sub billed 4.500, master 4.320 = 4.500×0.96) is
> the load-bearing proof.
>
> ⚠️ **Tension with `hype-cost-wall-20260917.md`, stated rather than resolved:** this document
> argues the fee is only **0.35× the average 1-minute move** so a minute-horizon maker has room.
> The cost agent measured **passive** spread-capture at **−0.356 bps/fill BEFORE any fee** (t=−92,
> 0 of 72 cells positive) — i.e. adverse selection alone kills it. **Both are right about
> different objects**: passive quoting is dead; a *signal-driven* maker still faces the
> required-IC table, which puts 1m maker/maker at **IC 0.30** (very hard) and 1h at **0.046**
> (the only open corner). Do not read "8 bps to play for" as "8 bps available to a passive quote".
