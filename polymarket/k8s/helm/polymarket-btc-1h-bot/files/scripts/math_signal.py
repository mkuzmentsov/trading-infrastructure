"""
Latency arbitrage signal for BTC binary up/down bar markets.

In the 5m chart this exploits the lag between Binance and the Chainlink
oracle Polymarket settles 5m bars on. In the 15m chart Polymarket already
settles on Binance, so the same math is applied directly to the Binance
feed and the "lead time" comes from book staleness rather than oracle lag.
...
def _fair_p_up(
    ...
    time_frac = max(seconds_left / float(BAR_DURATION_SECS), 1e-6)
