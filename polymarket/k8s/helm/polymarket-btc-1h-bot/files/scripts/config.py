...
LOOP_INTERVAL = int(os.getenv("LOOP_INTERVAL_SECS", "900"))
# Bar duration for the Polymarket up/down market we trade. 5m=300, 15m=900.
# Used by gamma slug discovery, math time_frac, and bar_open backfill.
BAR_DURATION_SECS = int(os.getenv("BAR_DURATION_SECS", "900"))
...
# RTDS / Chainlink feed is intentionally disabled in the 15m bot — the 15m
# Polymarket BTC market resolves on Binance data, so we run signal off Binance only.
POLYMARKET_RTDS_WS = os.getenv("POLYMARKET_RTDS_WS", "")
