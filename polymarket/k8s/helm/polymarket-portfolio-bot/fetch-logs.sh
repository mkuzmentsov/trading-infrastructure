#!/usr/bin/env bash
# Fetch logs for a pm-portfolio pod since it was started:
#   - Loki logs (via kubectl port-forward + logcli) -> loki.log
#   - /app/data/cycles.jsonl
#   - /app/data/seen_markets.json
set -euo pipefail

# --- Config ---------------------------------------------------------------
NAMESPACE="${NAMESPACE:-polymarket}"
POD_SELECTOR="${POD_SELECTOR:-pm-portfolio}"   # substring match on pod name
POD_NAME="${POD_NAME:-}"                        # override: exact pod name
OUT_DIR="${OUT_DIR:-./pm-portfolio-logs_$(date +%Y%m%d_%H%M%S)}"
...
echo "==> Copying /app/data/* from pod..."
for f in cycles.jsonl seen_markets.json; do
  if kubectl -n "$NAMESPACE" exec "$POD_NAME" -- test -f "/app/data/$f" 2>/dev/null; then
    kubectl -n "$NAMESPACE" cp "$POD_NAME:/app/data/$f" "$OUT_DIR/$f" \
      && echo "    ok: $f" \
      || echo "    warn: failed copying $f"
  else
    echo "    skip: /app/data/$f not present"
  fi
done
