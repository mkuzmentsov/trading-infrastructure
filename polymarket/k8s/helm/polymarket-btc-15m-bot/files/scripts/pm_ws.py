...
On startup and whenever the current bar market rolls over, fetches
...
from gamma import fetch_btc_bar_market, get_market_window, get_up_down_tokens
...
async def run_pm_ws() -> None:
    ...
    while True:
        try:
            market = fetch_btc_bar_market()
            if not market:
                log.warning("No active BTC bar market — retry in 30s")
