...
def fetch_btc_bar_market() -> Optional[dict]:
    ...
    Bar duration comes from config.BAR_DURATION_SECS (e.g. 300 for 5m, 900 for 15m).
    Slug pattern: btc-updown-{N}m-{boundary_ts}. Tries the current boundary
    first; if not found or already closed, falls back to the next boundary.
    """
    ...
    for offset_secs in (0, BAR_DURATION_SECS):
        ...
        slug = f"btc-updown-{minutes}m-{ts}"
        try:
