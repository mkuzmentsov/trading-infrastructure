"""
Polymarket BTC 15m direction bot — Binance-fed, AI-disabled variant.

Strategy:
- Uses the Binance BTC/USDT aggTrade feed as the sole price reference.
  The 15m up/down market resolves on Binance data (no Chainlink oracle).
- When the PM book lags the Binance price within a bar, buys the cheap
  side (UP or DOWN token).
...
- AI gates (entry/exit classifier, next-bar model) are disabled via env.
"""
...
# Last exit reason per (cid, direction). Used by BLOCK_REENTRY_AFTER_REASONS
...
def _log_signal_debug(context: str, signal, cash_amount: float, seconds_left: int) -> None:
    bp = _binance_price_if_fresh()
    log.info(
        "Signal eval [%s] inputs: cash=%.2f secs_left=%d open=%.2f btc=%.2f binance=%.2f ret30=%.6f ret60=%.6f sigma5m=%.6f up_bid=%.3f up_ask=%.3f down_bid=%.3f down_ask=%.3f",
        context,
        cash_amount,
        seconds_left,
        btc_state.bar_open,
    ...
    while asyncio.get_event_loop().time() < deadline:
        if btc_state.ready and pm_state.ready:
            if binance_state.ready:
                ...
                log.info("Core feeds ready (binance_state still connecting — btc_state already on Binance)")
            return True
...
async def _try_enter(clob, balance: float) -> None:
    ...
    if BLOCK_REENTRY_AFTER_REASONS:
        last_reason = _last_exit_reason_by_dir.get((pm_state.condition_id, direction))
        if last_reason in BLOCK_REENTRY_AFTER_REASONS:
            log.info(
                "Signal blocked by exit-reason reentry guard  dir=%s  last_exit_reason=%s",
                direction, last_reason,
            )
            _entry_confirmation["condition_id"] = ""
            _entry_confirmation["action"] = ""
            _entry_confirmation["count"] = 0
...
async def main() -> None:
    ...
    if not ready:
        ...
        log.info("Feeds ready  Binance BTC=$%.2f  PM=%s", btc_state.current_price, pm_state.question[:50])
    ...
    while True:
        ...
        if current_boundary > 0 and now >= current_boundary and current_boundary > last_cleanup_boundary:
            ...
            stale_reason_keys = [key for key in _last_exit_reason_by_dir if key[0] != pm_state.condition_id]
