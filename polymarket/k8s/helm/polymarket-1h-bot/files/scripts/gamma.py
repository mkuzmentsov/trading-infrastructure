...
# Asset prefix for the hourly slug. Polymarket uses different prefixes per asset
# but the same date/hour suffix:
#   BTC  → "bitcoin-up-or-down"
#   ETH  → "ethereum-up-or-down"
#   XRP  → "xrp-up-or-down"
#   SOL  → "solana-up-or-down"
# Default keeps the BTC behavior so charts that don't set this still work.
_MARKET_SLUG_PREFIX = os.getenv("MARKET_SLUG_PREFIX", "bitcoin-up-or-down")
...
def _hourly_slug(boundary_ts: int) -> str:
    ...
    dates and 12-hour clock with am/pm. Asset prefix comes from the
    MARKET_SLUG_PREFIX env (e.g. ``bitcoin-up-or-down``, ``ethereum-up-or-down``).
    Example: 2026-04-29T10:00:00Z (= 6 AM EDT), prefix bitcoin-up-or-down
    → ``bitcoin-up-or-down-april-29-2026-6am-et``.
    """
    ...
    if hour_12 == 0:
        hour_12 = 12
    return f"{_MARKET_SLUG_PREFIX}-{month}-{dt_et.day}-{dt_et.year}-{hour_12}{ampm}-et"
