...
def main():
    ...
    # Load wallets of interest — appended to results after top-N is selected
    ...
    # Append wallets of interest not already present in the top-N results
    if wallets_of_interest:
        result_addrs = {t.address.lower() for t, _, _ in top_with_data}
        woi_missing = [w for w in wallets_of_interest if w not in trader_by_addr]
        if woi_missing:
            print(f"WARNING: {len(woi_missing)} wallet(s) of interest not found in leaderboard: {', '.join(woi_missing)}")
        woi_to_append = [(w, trader_by_addr[w]) for w in wallets_of_interest if w in trader_by_addr and w not in result_addrs]
        if woi_to_append:
            print(f"\nAppending {len(woi_to_append)} wallet(s) of interest not in top {args.top}...")
            for w, t in woi_to_append:
                if args.check_positions:
                    perp = fetch_perp_state(t.address, t)
                    _time.sleep(0.2)
                    activity = fetch_activity_stats(t.address)
                    ...
                    perp = None
                    activity = None
                top_with_data.append((t, activity, perp))
                print(f"  appended {t.address} ({t.label()})")
